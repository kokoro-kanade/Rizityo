...
namespace Rizityo::Graphics::D3D12::Content
{
    ...
    namespace
    {
        ...
        ID::IDType CreatePSO_IfNeeded(const uint8* const streamPtr, uint64 alignedStreamSize, [[maybe_unused]] bool isDepth)
        {
            ...
            PipelineStates.emplace_back(Helper::CreatePipelineState(stream, sizeof(Helper::D3D12PipelineStateSubobjectStream)));
        ...
        PSO_ID CreatePSO(ID::IDType materialID, D3D12_PRIMITIVE_TOPOLOGY primitiveTopology, uint32 elementsType)
        {
            ...
            constexpr uint64 aligned_stream_size{ Math::AlignSizeUp<sizeof(uint64)>(sizeof(Helper::D3D12PipelineStateSubobjectStream)) };
            uint8* const stream_ptr{ (uint8* const)alloca(aligned_stream_size) };
            ...
            for (uint32 i = 0; i < ShaderType::Count; i++)
            {
                if (flags & (1 << i))
                {
                    Rizityo::Content::CompiledShaderPtr shader{ Rizityo::Content::GetShader(material.ShaderIDs()[shader_index]) };
                    assert(shader);
            ...
            PSO_ID idPair{};
            idPair.GPassPSO_ID = CreatePSO_IfNeeded(stream_ptr, aligned_stream_size, false);

            stream.PS = D3D12_SHADER_BYTECODE{};
            stream.DepthStencil1 = Helper::DepthState.Enabled;
            idPair.DepthPSO_ID = CreatePSO_IfNeeded(stream_ptr, aligned_stream_size, true);
